...
function runRelayAction(action) {
  const allowed = new Set(["relay-enable", "relay-disable", "relay-tunnel-up", "relay-tunnel-down", "relay-restart", "relay-on", "relay-off", "relay-restore"]);
...
async function handleApi(req, res) {
  ...
  if (req.method === "POST" && req.url === "/api/relay/action") {
    ...
    if (action === "relay-disable" || action === "relay-off") saveConfigPatch({ RELAY_ENABLED: "0" });
    const nextConfig = loadConfig();
    const relay = relayStatus(nextConfig);
    relay.last_result = result;
    return sendJson(res, 200, { ok: true, config: nextConfig, relay });
