...
function appPage() {
  return page("wg-captive admin", `<div class="app-shell"><aside class="sidebar"><div class="sidebar-brand"><button id="sidebarToggle" class="hamburger" type="button" aria-label="Toggle sidebar"><span></span><span></span><span></span></button><strong>WireGuard Admin</strong></div><div class="side-group"><span>MANAGEMENT</span><button class="tab active" data-tab="clients">Captive</button><button class="tab" data-tab="settings">Settings</button></div></aside><div class="main-shell"><main class="content"><section id="clientsTab"><div class="section-head"><div><h1>Captive</h1><p>Quan ly trang thai captive cua tung WireGuard client.</p></div><div class="summary-pill"><span id="clientCount">0</span> clients</div></div><div id="clientList" class="client-list"></div></section><section id="settingsTab" class="settings hidden"><div class="section-head"><div><h1>Settings</h1><p>Cau hinh portal va Telegram backup.</p></div></div><form id="settingsForm" class="settings-grid"><label>Portal IP<input name="PORTAL_IP" id="PORTAL_IP"></label><label>Server IP<input name="SERVER_PUBLIC_IP" id="SERVER_PUBLIC_IP"></label><label>Telegram bot token<input name="TELEGRAM_BOT_TOKEN" id="TELEGRAM_BOT_TOKEN"></label><label>Telegram chat ID<input name="TELEGRAM_CHAT_ID" id="TELEGRAM_CHAT_ID"></label><label class="wide">Auto backup times, comma separated HH:MM<input name="AUTO_BACKUP_TIMES" id="AUTO_BACKUP_TIMES" placeholder="02:00,14:00"></label><label class="check"><input type="checkbox" name="BLOCK_DNS" id="BLOCK_DNS"> <span>Block DNS 53</span></label><label class="check"><input type="checkbox" name="BLOCK_DOT" id="BLOCK_DOT"> <span>Block DNS-over-TLS 853</span></label><label class="check"><input type="checkbox" name="TELEGRAM_BACKUP_ENABLED" id="TELEGRAM_BACKUP_ENABLED"> <span>Send backup to Telegram</span></label><label class="check"><input type="checkbox" name="AUTO_BACKUP_ENABLED" id="AUTO_BACKUP_ENABLED"> <span>Enable auto backup</span></label><button type="submit" class="primary save-btn">Save settings</button></form><div class="backup-panel"><button id="backupBtn" class="primary">Backup now</button><form id="restoreForm" enctype="multipart/form-data"><input type="file" id="restoreFile" name="backup" accept=".tar.gz,.tgz,application/gzip"><button type="submit" class="ghost">Restore upload</button></form></div></section></main><footer class="footer">Powered by &#272;&#7841;i An VPN</footer></div></div><div id="toast"></div><script>${clientJs()}</script>`);
}
function css() { return `:root{--bg:#0b1220;--panel:#101827;--panel3:#0e1728;--line:#1e2a3d;--text:#f8fafc;--muted:#93a4bc;--blue:#2f6df6;--blue2:#1d4ed8;--green:#20c76a;--red2:#b91c1c;--shadow:0 10px 30px rgba(0,0,0,.18)}*{box-sizing:border-box}html,body{min-height:100%;margin:0;background:#0b1220;color:var(--text);font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.app-shell{min-height:100vh;display:flex;background:linear-gradient(180deg,#111827 0,#0b1220 230px)}.sidebar{width:260px;flex:0 0 260px;margin:14px 0 14px 14px;border:1px solid #17283a;border-radius:16px;background:linear-gradient(180deg,#0c1b24,#101827 58%,#0b1220);box-shadow:0 18px 50px rgba(0,0,0,.24);padding:18px}.sidebar-brand{display:flex;align-items:center;gap:14px;padding:4px 2px 18px;border-bottom:1px solid #1e3449}.sidebar-brand strong{font-size:20px;line-height:1;color:#fff;font-weight:900;white-space:nowrap}.hamburger{width:36px;display:grid;gap:5px;border:0;background:transparent;padding:7px 3px;cursor:pointer}.hamburger span{height:2px;background:#e5edf7;border-radius:999px}.hamburger:hover span{background:#33d1ff}.app-shell.sidebar-collapsed .sidebar{width:74px;flex-basis:74px;padding:18px 12px}.app-shell.sidebar-collapsed .sidebar-brand{justify-content:center;border-bottom:0;padding-bottom:0}.app-shell.sidebar-collapsed .sidebar-brand strong,.app-shell.sidebar-collapsed .side-group{display:none}.side-group{display:grid;gap:8px;padding-top:22px}.side-group>span{color:#92a1b4;font-size:12px;font-weight:900;letter-spacing:2px;margin:0 0 6px 2px}.side-group>span:before{content:"";display:inline-block;width:4px;height:14px;border-radius:999px;background:#33d1ff;margin-right:12px;vertical-align:-2px}.side-group button{width:100%;border:0;background:transparent;color:#cbd5e1;text-align:left;border-radius:10px;padding:12px 14px;font-size:16px;font-weight:750;cursor:pointer}.side-group button:hover{background:#132236;color:#fff}.side-group .tab.active{background:rgba(34,211,238,.12);box-shadow:inset 3px 0 0 #22d3ee;color:#e0faff}.main-shell{min-width:0;flex:1;display:flex;flex-direction:column}.primary,.ghost{border:0;border-radius:12px;padding:11px 16px;color:white;font-weight:800;font-size:14px;cursor:pointer;white-space:nowrap}.primary{background:var(--blue);box-shadow:0 10px 24px rgba(47,109,246,.2)}.primary:hover{background:var(--blue2)}.ghost{background:#364255;color:#fff}.ghost:hover{background:#46546a}.content{width:100%;max-width:1080px;margin:0 auto;padding:22px 20px 26px;flex:1}.section-head{display:flex;align-items:end;justify-content:space-between;gap:16px;margin:0 0 14px}.section-head h1{margin:0;font-size:26px;letter-spacing:0}.section-head p{margin:5px 0 0;color:var(--muted);font-size:14px}.summary-pill{border:1px solid var(--line);background:#0d1626;color:#dbeafe;border-radius:999px;padding:8px 12px;font-weight:800;font-size:14px}.client-list{display:grid;gap:10px}.client-row{min-height:72px;background:var(--panel);border:1px solid #132036;border-radius:12px;box-shadow:var(--shadow);display:grid;grid-template-columns:minmax(180px,1fr) 150px 64px;align-items:center;gap:14px;padding:13px 16px}.name{font-size:19px;font-weight:900;line-height:1.05;margin-bottom:5px}.sub{display:flex;gap:12px;align-items:center;color:#9bc9ff;font-size:15px}.ip{color:#9bc9ff}.status{color:var(--muted);font-size:12px}.status strong{display:block;color:var(--text);font-size:15px;font-weight:850;margin-bottom:3px}.status.allowed strong{color:var(--green)}.status.blocked strong{color:#ff6b6b}.avatar-wrap,.avatar,.dot{display:none}.switch{justify-self:end;position:relative;display:inline-block;width:58px;height:31px}.switch input{display:none}.slider{position:absolute;inset:0;border-radius:999px;background:#334155;transition:.18s;box-shadow:inset 0 0 0 1px rgba(255,255,255,.06)}.slider:before{content:"";position:absolute;width:23px;height:23px;right:4px;top:4px;background:white;border-radius:50%;transition:.18s;box-shadow:0 3px 10px rgba(0,0,0,.25)}.switch input:checked+.slider{background:var(--green)}.switch input:not(:checked)+.slider{background:var(--red2)}.switch input:not(:checked)+.slider:before{right:31px}.empty{padding:30px 20px;background:var(--panel);border:1px solid var(--line);border-radius:12px;color:var(--muted);text-align:center}.hidden{display:none}.settings{background:var(--panel3);border:1px solid var(--line);border-radius:14px;box-shadow:var(--shadow);padding:20px}.settings-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.settings label{display:grid;gap:7px;color:#c7d2e2;font-size:12px;font-weight:800}.settings input{border:1px solid #263449;background:#0a1220;color:var(--text);border-radius:10px;padding:11px 12px;font:inherit;min-width:0}.settings input:focus{outline:2px solid rgba(47,109,246,.45);border-color:#3b82f6}.settings .check{display:flex;align-items:center;gap:9px;background:#0a1220;border:1px solid #263449;border-radius:10px;padding:11px 12px}.settings .check input{width:16px;height:16px}.wide{grid-column:1/-1}.save-btn{justify-self:start;margin-top:2px}.backup-panel{display:flex;align-items:center;gap:10px;margin-top:16px;flex-wrap:wrap}.backup-panel form{display:flex;gap:9px;align-items:center;flex-wrap:wrap}.backup-panel input[type=file]{color:var(--muted);max-width:100%}.footer{padding:12px 16px;color:#7890bc;text-align:center;font-size:15px;border-top:1px solid rgba(30,42,61,.45);background:#0b1220}#toast{position:fixed;right:20px;bottom:20px;max-width:340px;padding:12px 14px;border-radius:10px;background:#e5e7eb;color:#0f172a;opacity:0;transform:translateY(10px);transition:.18s ease;font-weight:750;z-index:20}#toast.show{opacity:1;transform:translateY(0)}.login-shell{min-height:100vh;display:grid;place-items:center;padding:18px;background:linear-gradient(180deg,#111827,#0b1220)}.login-card{width:min(380px,calc(100vw - 28px));background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:24px;box-shadow:var(--shadow)}.login-card h1{font-size:30px;margin:0 0 3px}.login-card p{color:var(--muted);margin:0 0 10px}.login-card input,.login-card button{width:100%;margin-top:12px}.login-card input{border:1px solid #263449;background:#0a1220;color:var(--text);border-radius:10px;padding:11px 12px;font:inherit}.login-card button{border:0;border-radius:12px;background:var(--blue);color:white;padding:11px 14px;font-weight:850;cursor:pointer}.alert{background:#451a1a;color:#fecaca;border-radius:10px;padding:9px 10px;margin-top:12px}@media(max-width:820px){.app-shell{display:block}.sidebar{width:auto;margin:10px 10px 0;border-radius:14px;padding:12px}.sidebar-brand{padding:2px 2px 12px}.sidebar-brand strong{font-size:20px}.hamburger{width:30px}.app-shell.sidebar-collapsed .sidebar{width:auto;flex-basis:auto;padding:12px}.app-shell.sidebar-collapsed .sidebar-brand{justify-content:flex-start}.side-group{padding-top:12px;grid-template-columns:1fr 1fr}.side-group>span{grid-column:1/-1}.side-group button{text-align:center;padding:11px 10px;font-size:15px}.content{padding:18px 14px 22px}.section-head{align-items:flex-start;flex-direction:column;margin-bottom:12px}.section-head h1{font-size:25px}.section-head p{font-size:14px}.client-list{gap:10px}.client-row{grid-template-columns:1fr 62px;min-height:74px;padding:14px 15px}.name{font-size:20px}.sub{font-size:15px}.status{display:none}.switch{grid-column:2;grid-row:1;width:62px;height:33px}.switch .slider:before{width:25px;height:25px}.switch input:not(:checked)+.slider:before{right:33px}.settings{padding:16px}.settings-grid{grid-template-columns:1fr}.settings label{font-size:13px}.settings input{padding:12px 13px}.primary,.ghost{padding:11px 14px;font-size:15px}.footer{font-size:14px;padding:10px 14px}.login-card{padding:22px}}`; }
function clientJs() { return `let state={clients:[],config:{}};let toggleSeq=0;const pendingToggles={};const $=(id)=>document.getElementById(id);function initSidebar(){const shell=document.querySelector('.app-shell');const btn=$("sidebarToggle");if(!shell||!btn)return;let saved=false;try{saved=localStorage.getItem('sidebarCollapsed')==='1'}catch{}shell.classList.toggle('sidebar-collapsed',saved);btn.addEventListener('click',()=>{const collapsed=!shell.classList.contains('sidebar-collapsed');shell.classList.toggle('sidebar-collapsed',collapsed);try{localStorage.setItem('sidebarCollapsed',collapsed?'1':'0')}catch{}})}function toast(message){const box=$("toast");box.textContent=message;box.classList.add("show");setTimeout(()=>box.classList.remove("show"),2200)}async function api(path,options={}){const res=await fetch(path,{headers:{"Content-Type":"application/json",...(options.headers||{})},...options});const data=await res.json();if(!res.ok||data.ok===false)throw new Error(data.error||"Request failed");return data}function renderClients(){const list=$("clientList");const clients=state.clients;const count=$("clientCount");if(count)count.textContent=clients.length;if(!clients.length){list.innerHTML='<div class="empty">No clients found in wg0.json.</div>';return}list.innerHTML=clients.map((c)=>{const pending=pendingToggles[c.ip];const captive=c.captive;return '<div class="client-row"><div><div class="name">'+escapeHtml(c.name)+'</div><div class="sub"><span class="ip">'+c.ip+'</span></div></div><div class="status '+(captive?'blocked':'allowed')+'"><strong>'+(captive?'Bi khoa':'Dang hoat dong')+'</strong><span>'+(pending?'updating':(captive?'captive on':'internet open'))+'</span></div><label class="switch" title="Toggle captive"><input type="checkbox" data-ip="'+c.ip+'" '+(captive?'':'checked')+' '+(pending?'disabled':'')+'><span class="slider"></span></label></div>'}).join('')}function renderSettings(){for(const key of ['PORTAL_IP','SERVER_PUBLIC_IP','TELEGRAM_BOT_TOKEN','TELEGRAM_CHAT_ID','AUTO_BACKUP_TIMES'])if($(key))$(key).value=state.config[key]||'';if($('TELEGRAM_BACKUP_ENABLED'))$('TELEGRAM_BACKUP_ENABLED').checked=state.config.TELEGRAM_BACKUP_ENABLED!=='0';if($('AUTO_BACKUP_ENABLED'))$('AUTO_BACKUP_ENABLED').checked=state.config.AUTO_BACKUP_ENABLED==='1';if($('BLOCK_DNS'))$('BLOCK_DNS').checked=state.config.BLOCK_DNS==='1';if($('BLOCK_DOT'))$('BLOCK_DOT').checked=state.config.BLOCK_DOT==='1'}function escapeHtml(v){return String(v).replace(/[&<>"']/g,(ch)=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[ch]))}function timeAgo(iso){const t=Date.parse(iso);if(!t)return '';const diff=Math.max(0,Date.now()-t);const s=Math.floor(diff/1000);if(s<60)return s+' seconds ago';const m=Math.floor(s/60);if(m<60)return m+' minutes ago';const h=Math.floor(m/60);if(h<24)return h+' hours ago';return Math.floor(h/24)+' days ago'}async function load(){const data=await api('/api/state');state=data;renderClients();renderSettings()}document.querySelectorAll('.tab').forEach((btn)=>btn.addEventListener('click',()=>{document.querySelectorAll('.tab').forEach((b)=>b.classList.remove('active'));btn.classList.add('active');$('clientsTab').classList.toggle('hidden',btn.dataset.tab!=='clients');$('settingsTab').classList.toggle('hidden',btn.dataset.tab!=='settings')}));$('clientList').addEventListener('change',async(e)=>{if(!e.target.matches('input[type="checkbox"]'))return;const input=e.target;const ip=input.dataset.ip;const checked=input.checked;const previous=!checked;input.checked=previous;const seq=++toggleSeq;pendingToggles[ip]={checked,seq};renderClients();try{const data=await api('/api/toggle',{method:'POST',body:JSON.stringify({ip,captive:!checked})});if(!pendingToggles[ip]||pendingToggles[ip].seq!==seq)return;state.clients=data.clients||state.clients;delete pendingToggles[ip];toast(checked?'Client active':'Client expired');renderClients()}catch(error){if(pendingToggles[ip]&&pendingToggles[ip].seq===seq)delete pendingToggles[ip];toast(error.message);renderClients()}});$('settingsForm').addEventListener('submit',async(e)=>{e.preventDefault();const payload=Object.fromEntries(new FormData(e.target).entries());payload.TELEGRAM_BACKUP_ENABLED=$('TELEGRAM_BACKUP_ENABLED').checked?'1':'0';payload.AUTO_BACKUP_ENABLED=$('AUTO_BACKUP_ENABLED').checked?'1':'0';payload.BLOCK_DNS=$('BLOCK_DNS').checked?'1':'0';payload.BLOCK_DOT=$('BLOCK_DOT').checked?'1':'0';const data=await api('/api/settings',{method:'POST',body:JSON.stringify(payload)});state.config=data.config;renderSettings();toast('Settings saved')});$('backupBtn').addEventListener('click',async()=>{await api('/api/backup',{method:'POST',body:'{}'});toast('Backup created')});$('restoreForm').addEventListener('submit',async(e)=>{e.preventDefault();const file=$('restoreFile').files[0];if(!file){toast('Choose backup file');return}const form=new FormData();form.append('backup',file);const res=await fetch('/api/restore',{method:'POST',body:form});const data=await res.json();if(!res.ok||data.ok===false)throw new Error(data.error||'Restore failed');toast('Restore complete');await load()});load().catch((error)=>toast(error.message));`; }
