...
function relayValueClass(value){const v=String(value||'').toLowerCase();if(/\\b(up|enabled|yes|ok|active|running)\\b/.test(v))return 'good';if(/\\b(down|disabled|no|failed|error|missing|not found)\\b/.test(v))return 'bad';if(/\\b(warn|partial|unknown)\\b/.test(v))return 'warn';return 'neutral'}
function relayStatusParts(text){const lines=String(text||'').split(/\\r?\\n/).map((line)=>line.trim()).filter(Boolean);const config=[];const status=[];let target=config;for(const line of lines){if(/^===/.test(line)){target=status;status.push({section:line.replace(/=/g,'').trim()});continue}const match=line.match(/^([^:]+):\\s*(.*)$/);const item=match?{label:match[1].replace(/_/g,' '),value:match[2]||'-'}:{label:'detail',value:line};target.push(item)}return {config,status}}
function renderRelayRows(id,items,emptyText){const box=$(id);if(!box)return;if(!items.length){box.innerHTML='<div class="relay-empty">'+emptyText+'</div>';return}box.innerHTML=items.map((item)=>item.section?'<div class="relay-section">'+escapeHtml(item.section)+'</div>':'<div class="relay-kv-row"><span>'+escapeHtml(item.label)+'</span><strong class="'+relayValueClass(item.value)+'">'+escapeHtml(item.value)+'</strong></div>').join('')}
function renderRelay(){const relay=state.relay||{};if($('RELAY_TUNNEL'))$('RELAY_TUNNEL').checked=!!relay.tunnel_up;if($('RELAY_ROUTE'))$('RELAY_ROUTE').checked=!!relay.route_enabled;const parts=relayStatusParts(relay.status||'');renderRelayRows('relayConfigCard',parts.config,'No exit config');renderRelayRows('relayHealthCard',parts.status,'No relay status yet')}
