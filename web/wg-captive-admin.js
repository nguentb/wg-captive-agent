...
function portalHostFromUrl(value) {
...
function isIpv4(value) {
  return /^\d{1,3}(\.\d{1,3}){3}$/.test(String(value || "")) && String(value).split(".").every((part) => Number(part) >= 0 && Number(part) <= 255);
}

async function resolvePortalIp(portalUrl) {
...
function dbListApiTokens(config) {
  if (!dbAvailable(config)) return [];
  return sqliteRows(config, "SELECT id, name, prefix, created_at FROM api_tokens ORDER BY created_at DESC");
}

function dbCreateApiToken(config, name) {
  if (!dbAvailable(config)) throw new Error("SQLite is not available");
  const id = crypto.randomBytes(8).toString("hex");
  const token = "wgc_" + crypto.randomBytes(32).toString("base64url");
  const label = String(name || "API token").trim().slice(0, 80) || "API token";
  const prefix = token.slice(0, 12);
  sqliteRaw(config, `INSERT INTO api_tokens(id,name,token,prefix,created_at) VALUES(${sqlQuote(id)},${sqlQuote(label)},${sqlQuote(token)},${sqlQuote(prefix)},CURRENT_TIMESTAMP)`);
  return { item: { id, name: label, prefix, created_at: new Date().toISOString() }, token };
}

function dbGetApiToken(config, id) {
  if (!dbAvailable(config) || !/^[a-f0-9]{16}$/.test(String(id || ""))) return null;
  return sqliteRows(config, `SELECT id, name, token, prefix, created_at FROM api_tokens WHERE id=${sqlQuote(id)} LIMIT 1`)[0] || null;
}

function dbDeleteApiToken(config, id) {
  if (!dbAvailable(config) || !/^[a-f0-9]{16}$/.test(String(id || ""))) return false;
  sqliteRaw(config, `DELETE FROM api_tokens WHERE id=${sqlQuote(id)}`);
  return true;
}

function dbApiTokenMatches(config, token) {
  if (!token || !dbAvailable(config)) return false;
  return sqliteRows(config, `SELECT id FROM api_tokens WHERE token=${sqlQuote(token)} LIMIT 1`).length > 0;
}
...
function loadConfig() {
  ...
  if (!config.PORTAL_URL && config.PORTAL_IP) config.PORTAL_URL = `http://${config.PORTAL_IP}`;
  return config;
}
...
function isApiAuthenticated(req, config) {
  ...
  if (bearer && dbApiTokenMatches(config, bearer)) return true;
...
function initSidebar(){const shell=document.querySelector('.app-shell');const btn=$('sidebarToggle');const openBtn=$('sidebarOpen');const backdrop=$('sidebarBackdrop');if(!shell||!btn)return;const isMobile=()=>window.matchMedia('(max-width:820px)').matches;const setCollapsed=(collapsed)=>{shell.classList.toggle('sidebar-collapsed',collapsed);btn.setAttribute('aria-expanded',String(!collapsed));if(openBtn)openBtn.setAttribute('aria-expanded',String(!collapsed));try{localStorage.setItem('sidebarCollapsed',collapsed?'1':'0')}catch{}};let saved=false;try{saved=localStorage.getItem('sidebarCollapsed')==='1'}catch{}setCollapsed(isMobile()?true:saved);btn.addEventListener('click',()=>setCollapsed(!shell.classList.contains('sidebar-collapsed')));if(openBtn)openBtn.addEventListener('click',()=>setCollapsed(false));if(backdrop)backdrop.addEventListener('click',()=>setCollapsed(true));document.querySelectorAll('.tab').forEach((tab)=>tab.addEventListener('click',()=>{if(isMobile())setCollapsed(true)}))}
function initScrollHoverGuard(){let timer=0;const mark=()=>{document.body.classList.add('is-scrolling');clearTimeout(timer);timer=setTimeout(()=>document.body.classList.remove('is-scrolling'),520)};window.addEventListener('wheel',mark,{passive:true});window.addEventListener('scroll',mark,{passive:true})}
...
const API_BASE='/api/v1';const API_ENDPOINTS={state:API_BASE+'/state',sync:API_BASE+'/sync',clientStatus:(ip)=>API_BASE+'/clients/'+encodeURIComponent(ip)+'/status',clientExpiry:(ip)=>API_BASE+'/clients/'+encodeURIComponent(ip)+'/expiry',settings:API_BASE+'/settings',tokens:API_BASE+'/tokens',token:(id)=>API_BASE+'/tokens/'+encodeURIComponent(id),tokenCopy:(id)=>API_BASE+'/tokens/'+encodeURIComponent(id)+'/copy',relayAction:API_BASE+'/relay/action',relayImport:API_BASE+'/relay/import',backup:API_BASE+'/backup',restore:API_BASE+'/restore',expiryCheck:API_BASE+'/expiry/check'};async function api(path,options={}){const res=await fetch(path,{headers:{'Content-Type':'application/json',...(options.headers||{})},...options});const data=await res.json();if(!res.ok||data.ok===false)throw new Error(data.error||'Request failed');return data}
...
function renderApiTokens(){const box=$('apiTokenList');if(!box)return;const tokens=state.api_tokens||[];if(!tokens.length){box.innerHTML='<div class="api-token-empty">No API tokens yet.</div>';return}box.innerHTML=tokens.map((item)=>'<div class="api-token-row" data-id="'+escapeHtml(item.id)+'"><div><div class="api-token-name">'+escapeHtml(item.name||'API token')+'</div><span class="api-token-meta">'+escapeHtml(item.prefix||'wgc_')+'... · '+escapeHtml(item.created_at||'')+'</span></div><button type="button" class="ghost compact api-token-copy" data-id="'+escapeHtml(item.id)+'">Copy</button><button type="button" class="ghost danger compact api-token-delete" data-id="'+escapeHtml(item.id)+'">Delete</button></div>').join('')}
async function copyText(value){try{await navigator.clipboard.writeText(value);return true}catch{const input=document.createElement('textarea');input.value=value;document.body.appendChild(input);input.select();const ok=document.execCommand('copy');input.remove();return ok}}
function renderSettings(){for(const key of ['SERVER_NAME','PORTAL_URL','SERVER_PUBLIC_IP','RELAY_CLIENT_SUBNET','TELEGRAM_BOT_TOKEN','TELEGRAM_CHAT_ID','AUTO_BACKUP_TIMES'])if($(key))$(key).value=state.config[key]||'';if($('TELEGRAM_BACKUP_ENABLED'))$('TELEGRAM_BACKUP_ENABLED').checked=state.config.TELEGRAM_BACKUP_ENABLED!=='0';if($('AUTO_BACKUP_ENABLED'))$('AUTO_BACKUP_ENABLED').checked=state.config.AUTO_BACKUP_ENABLED==='1';if($('BLOCK_DNS'))$('BLOCK_DNS').checked=state.config.BLOCK_DNS==='1';if($('BLOCK_DOT'))$('BLOCK_DOT').checked=state.config.BLOCK_DOT==='1';updateAutoBackupCard();renderApiTokens()}
...
$('apiTokenForm').addEventListener('submit',async(e)=>{e.preventDefault();const btn=e.target.querySelector('button');btn.disabled=true;try{const name=$('apiTokenName').value.trim();const data=await api(API_ENDPOINTS.tokens,{method:'POST',body:JSON.stringify({name})});state.api_tokens=data.tokens||[];$('apiTokenName').value='';renderApiTokens();if(data.token){await copyText(data.token);toast('Token created and copied')}}catch(error){toast(error.message)}finally{btn.disabled=false}});
$('apiTokenList').addEventListener('click',async(e)=>{const copy=e.target.closest('.api-token-copy');const del=e.target.closest('.api-token-delete');if(copy){copy.disabled=true;try{const data=await api(API_ENDPOINTS.tokenCopy(copy.dataset.id));await copyText(data.token);toast('Token copied')}catch(error){toast(error.message)}finally{copy.disabled=false}return}if(del){if(!confirm('Delete this API token?'))return;del.disabled=true;try{const data=await api(API_ENDPOINTS.token(del.dataset.id),{method:'DELETE'});state.api_tokens=data.tokens||[];renderApiTokens();toast('Token deleted')}catch(error){toast(error.message)}finally{del.disabled=false}}});
...
initSidebar();initScrollHoverGuard();load().catch((error)=>toast(error.message));`; }
...
async function handleApi(req, res) {
  ...
  if (req.method === "GET" && pathname === "/api/v1") {
    return sendJson(res, 200, { ok: true, version: "v1", auth: "Bearer ADMIN_API_TOKEN or admin cookie", endpoints: {
      ...
      client_get: "GET /api/v1/clients/{ip}",
      ...
      tokens_get: "GET /api/v1/tokens",
      tokens_create: "POST /api/v1/tokens { name }",
      token_copy: "GET /api/v1/tokens/{id}/copy",
  ...
  if (req.method === "GET" && (pathname === "/api/v1/state" || pathname === "/api/state")) {
    return sendJson(res, 200, { ok: true, config, clients: loadClients(config), relay: relayStatus(config), api_tokens: dbListApiTokens(config) });
  }
  if (req.method === "GET" && pathname === "/api/v1/clients") return sendJson(res, 200, await clientsPayload());
  const clientGetMatch = pathname.match(/^\/api\/v1\/clients\/([^/]+)$/);
  if (req.method === "GET" && clientGetMatch) {
    const ip = decodeURIComponent(clientGetMatch[1]);
    if (!isValidIp(ip)) return sendJson(res, 400, { ok: false, error: "Invalid IP address" });
    const client = loadClients(config).find((item) => item.ip === ip);
    if (!client) return sendJson(res, 404, { ok: false, error: "Client not found", ip });
  ...
  if (req.method === "GET" && pathname === "/api/v1/tokens") {
    if (!isAuthenticated(req)) return sendJson(res, 401, { ok: false, error: "Admin session required" });
    return sendJson(res, 200, { ok: true, tokens: dbListApiTokens(config) });
  }

  if (req.method === "POST" && pathname === "/api/v1/tokens") {
    if (!isAuthenticated(req)) return sendJson(res, 401, { ok: false, error: "Admin session required" });
    const body = await jsonBody();
    const created = dbCreateApiToken(config, body.name);
    return sendJson(res, 200, { ok: true, token: created.token, item: created.item, tokens: dbListApiTokens(config) });
  }

  const tokenCopyMatch = pathname.match(/^\/api\/v1\/tokens\/([^/]+)\/copy$/);
  if (req.method === "GET" && tokenCopyMatch) {
    if (!isAuthenticated(req)) return sendJson(res, 401, { ok: false, error: "Admin session required" });
    const item = dbGetApiToken(config, decodeURIComponent(tokenCopyMatch[1]));
    if (!item) return sendJson(res, 404, { ok: false, error: "Token not found" });
    return sendJson(res, 200, { ok: true, token: item.token });
  }

  const tokenDeleteMatch = pathname.match(/^\/api\/v1\/tokens\/([^/]+)$/);
  if (req.method === "DELETE" && tokenDeleteMatch) {
    if (!isAuthenticated(req)) return sendJson(res, 401, { ok: false, error: "Admin session required" });
    dbDeleteApiToken(config, decodeURIComponent(tokenDeleteMatch[1]));
    return sendJson(res, 200, { ok: true, tokens: dbListApiTokens(config) });
  }

  if (req.method === "POST" && (pathname === "/api/v1/settings" || pathname === "/api/settings")) {
    const body = await jsonBody();
    const keys = ["PORTAL_URL","PORTAL_IP","SERVER_PUBLIC_IP","RELAY_CLIENT_SUBNET","BLOCK_DNS","BLOCK_DOT","TELEGRAM_BOT_TOKEN","TELEGRAM_CHAT_ID","TELEGRAM_BACKUP_ENABLED","AUTO_BACKUP_ENABLED","AUTO_BACKUP_TIMES","EXPIRY_CHECK_TIMES","EXPIRY_CHECK_ENABLED","ADMIN_API_TOKEN"];
    const patch = {};
    for (const key of keys) if (body[key] !== undefined) patch[key] = String(body[key]).trim();
    if (patch.PORTAL_URL !== undefined) {
      try {
        const resolvedIp = await resolvePortalIp(patch.PORTAL_URL);
        if (!resolvedIp) return sendJson(res, 400, { ok: false, error: "Cannot resolve Portal URL" });
        patch.PORTAL_IP = resolvedIp;
      } catch (error) {
        return sendJson(res, 400, { ok: false, error: `Cannot resolve Portal URL: ${error.message}` });
      }
    }
    saveConfigPatch(patch);
    if (body.EXPIRY_CHECK_TIMES !== undefined || body.EXPIRY_CHECK_ENABLED !== undefined) scheduleExpiryCheckLoop();
    const firewallKeys = ["PORTAL_URL","PORTAL_IP","BLOCK_DNS","BLOCK_DOT"];
    const result = firewallKeys.some((key) => body[key] !== undefined) ? await queueAgentSync() : undefined;
    return sendJson(res, 200, { ok: true, config: loadConfig(), result });
  }
