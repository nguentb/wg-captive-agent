...
function renderClients(){const list=$('clientList');const clients=state.clients;const count=$('clientCount');if(count)count.textContent=clients.length;if(!clients.length){list.innerHTML='<div class="empty">No clients found in wg0.json.</div>';return}list.innerHTML=clients.map((c)=>{const pending=pendingToggles[c.ip];const checked=pending?pending.previousChecked:!c.captive;const captive=!checked;const expiryText=formatExpiry(c.expires_at);const expClass=expiryClass(c.expires_at);const pickerValue=toDateTimeLocal(c.expires_at);return '<div class="client-row" data-ip="'+c.ip+'"><div><div class="name-line"><div class="name">'+escapeHtml(c.name)+'</div><span class="expiry-badge '+expClass+'">'+escapeHtml(expiryText)+'</span></div><div class="sub"><span class="ip">'+c.ip+'</span></div></div><div class="status '+(captive?'blocked':'allowed')+'"><strong>'+(captive?'Bi khoa':'Dang hoat dong')+'</strong><span>'+(pending?'updating':(captive?'captive on':'internet open'))+'</span></div><button type="button" class="ghost renew-open" data-ip="'+c.ip+'">Gia han</button><label class="switch '+(pending?'is-pending':'')+'" title="Toggle captive"><input type="checkbox" data-ip="'+c.ip+'" '+(checked?'checked':'')+' '+(pending?'disabled':'')+'><span class="slider"></span></label><div class="renew-panel hidden" data-panel-ip="'+c.ip+'"><div class="renew-quick"><button type="button" class="ghost renew-quick-btn" data-ip="'+c.ip+'" data-months="1">+1 thang</button><button type="button" class="ghost renew-quick-btn" data-ip="'+c.ip+'" data-months="3">+3 thang</button><button type="button" class="ghost renew-quick-btn" data-ip="'+c.ip+'" data-months="6">+6 thang</button><button type="button" class="ghost renew-quick-btn" data-ip="'+c.ip+'" data-months="12">+1 nam</button></div><form class="renew-manual" data-ip="'+c.ip+'"><label>Thoi gian het han<div class="date-display" data-ip="'+c.ip+'">'+escapeHtml(formatDateTimeInput(pickerValue))+'</div><input type="hidden" name="expires_at" value="'+pickerValue+'"></label><div class="date-picker" data-picker-ip="'+c.ip+'"></div><button type="submit" class="primary">Luu thoi gian</button></form></div></div>'}).join('');document.querySelectorAll('.date-picker').forEach((box)=>renderDatePicker(box,box.dataset.pickerIp))}
function pickerDate(ip){const input=document.querySelector('.renew-manual[data-ip="'+ip+'"] input[name="expires_at"]');const date=input?.value?new Date(input.value):new Date();return Number.isFinite(date.getTime())?date:new Date()}
function setPickerDate(ip,date){const form=document.querySelector('.renew-manual[data-ip="'+ip+'"]');if(!form)return;form.querySelector('input[name="expires_at"]').value=toDateTimeLocal(date);const display=form.querySelector('.date-display');if(display)display.textContent=formatDateTimeInput(date);const picker=form.querySelector('.date-picker');if(picker)renderDatePicker(picker,ip)}
function renderDatePicker(box,ip){const selected=pickerDate(ip);const month=new Date(selected.getFullYear(),selected.getMonth(),1);const start=(month.getDay()+6)%7;const days=new Date(selected.getFullYear(),selected.getMonth()+1,0).getDate();let cells='';for(let i=0;i<start;i++)cells+='<span></span>';for(let d=1;d<=days;d++){const active=d===selected.getDate();cells+='<button type="button" class="date-day '+(active?'active':'')+'" data-day="'+d+'">'+d+'</button>'}box.innerHTML='<div class="picker-head"><button type="button" class="date-nav" data-step="-1">‹</button><strong>'+month.toLocaleString('vi-VN',{month:'long',year:'numeric'})+'</strong><button type="button" class="date-nav" data-step="1">›</button></div><div class="picker-week"><span>T2</span><span>T3</span><span>T4</span><span>T5</span><span>T6</span><span>T7</span><span>CN</span></div><div class="picker-grid">'+cells+'</div><label class="time-row">Gio het han<input type="time" value="'+pad2(selected.getHours())+':'+pad2(selected.getMinutes())+'"></label>'}
...
$('clientList').addEventListener('click',async(e)=>{const open=e.target.closest('.renew-open');if(open){const panel=document.querySelector('[data-panel-ip="'+open.dataset.ip+'"]');if(panel)panel.classList.toggle('hidden');return}const quick=e.target.closest('.renew-quick-btn');if(quick){setPickerDate(quick.dataset.ip,addClientMonths(pickerDate(quick.dataset.ip),Number(quick.dataset.months)));return}const nav=e.target.closest('.date-nav');if(nav){const picker=nav.closest('.date-picker');const ip=picker.dataset.pickerIp;setPickerDate(ip,addClientMonths(pickerDate(ip),Number(nav.dataset.step)));return}const day=e.target.closest('.date-day');if(day){const picker=day.closest('.date-picker');const ip=picker.dataset.pickerIp;const date=pickerDate(ip);date.setDate(Number(day.dataset.day));setPickerDate(ip,date);return}});
$('clientList').addEventListener('input',(e)=>{if(!e.target.matches('.time-row input'))return;const form=e.target.closest('.renew-manual');const [hh,mm]=String(e.target.value||'00:00').split(':').map(Number);const date=pickerDate(form.dataset.ip);date.setHours(Number.isFinite(hh)?hh:0,Number.isFinite(mm)?mm:0,0,0);setPickerDate(form.dataset.ip,date)});
