...
function initSidebar(){const shell=document.querySelector('.app-shell');const btn=$('sidebarToggle');const openBtn=$('sidebarOpen');if(!shell||!btn)return;const setCollapsed=(collapsed)=>{shell.classList.toggle('sidebar-collapsed',collapsed);btn.setAttribute('aria-expanded',String(!collapsed));if(openBtn)openBtn.setAttribute('aria-expanded',String(!collapsed));try{localStorage.setItem('sidebarCollapsed',collapsed?'1':'0')}catch{}};let saved=false;try{saved=localStorage.getItem('sidebarCollapsed')==='1'}catch{}setCollapsed(saved);btn.addEventListener('click',()=>setCollapsed(!shell.classList.contains('sidebar-collapsed')));if(openBtn)openBtn.addEventListener('click',()=>setCollapsed(false))}
