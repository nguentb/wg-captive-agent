...
function relayStatus(config) {
  ...
  return { enabled: config.RELAY_ENABLED === "1", conf_path: file, imported: fs.existsSync(file), updated_at: config.RELAY_UPDATED_AT || "", status };
}
...
function renderRelay(){const relay=state.relay||{};if($('RELAY_ENABLED'))$('RELAY_ENABLED').checked=!!relay.enabled;if($('RELAY_EXIT_CONF'))$('RELAY_EXIT_CONF').value=relay.conf_path||'';if($('RELAY_STATUS'))$('RELAY_STATUS').value=relay.imported?('Imported'+(relay.updated_at?' - '+relay.updated_at:'')):'No .conf imported';if($('relayStatusText'))$('relayStatusText').textContent=relay.status||'No relay status yet'}
...
$('relayForm').addEventListener('submit',async(e)=>{e.preventDefault();const data=await api('/api/relay/settings',{method:'POST',body:JSON.stringify({RELAY_ENABLED:$('RELAY_ENABLED').checked?'1':'0'})});state.config=data.config;state.relay=data.relay;renderRelay();toast('Relay saved')});
