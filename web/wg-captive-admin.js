...
function relayStatus(config) {
  ...
  return { enabled: routeEnabled, route_enabled: routeEnabled, tunnel_up: tunnelUp, conf_path: file, imported: fs.existsSync(file), updated_at: config.RELAY_UPDATED_AT || "", status };
}

function runRelayAction(action) {
  const allowed = new Set(["relay-tunnel-up", "relay-tunnel-down", "relay-restart", "relay-on", "relay-off", "relay-restore"]);
...
function renderRelay(){const relay=state.relay||{};if($('RELAY_ENABLED'))$('RELAY_ENABLED').checked=!!relay.enabled;if($('RELAY_EXIT_CONF'))$('RELAY_EXIT_CONF').textContent=relay.conf_path||'-';if($('RELAY_STATUS'))$('RELAY_STATUS').textContent=relay.imported?('Imported'+(relay.updated_at?' - '+relay.updated_at:'')):'No .conf imported';if($('relayStatusText'))$('relayStatusText').textContent=relay.status||'No relay status yet'}
...
$('RELAY_ENABLED').addEventListener('change',async(e)=>{const input=e.target;input.disabled=true;try{const data=await api('/api/relay/settings',{method:'POST',body:JSON.stringify({RELAY_ENABLED:input.checked?'1':'0'})});state.config=data.config;state.relay=data.relay;renderRelay();toast(input.checked?'Relay enabled':'Relay disabled')}catch(error){toast(error.message);input.checked=!input.checked}finally{input.disabled=false}});
...
async function handleApi(req, res) {
  ...
  if (req.method === "POST" && req.url === "/api/relay/action") {
    ...
    if (action === "relay-on") saveConfigPatch({ RELAY_ENABLED: "1" });
    if (action === "relay-off") saveConfigPatch({ RELAY_ENABLED: "0" });
    const nextConfig = loadConfig();
    const relay = relayStatus(nextConfig);
    relay.last_result = result;
    return sendJson(res, 200, { ok: true, config: nextConfig, relay });
