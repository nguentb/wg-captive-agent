...
async function createBackup(config, sendTelegram = true) {
  ...
  if (fs.existsSync(wgEasyFilePath(config))) fs.copyFileSync(wgEasyFilePath(config), path.join(workDir, "wg0"));
  else fs.writeFileSync(path.join(workDir, "wg0"), "{}\n");
  const metadata = {
...
async function handleApi(req, res) {
  ...
  if (req.method === "POST" && req.url === "/api/settings") {
    const body = JSON.parse(await readBody(req) || "{}");
    const keys = ["PORTAL_IP","SERVER_NAME","SERVER_PUBLIC_IP","WG_EASY_JSON","EXPIRED_FILE","BACKUP_DIR","TELEGRAM_BOT_TOKEN","TELEGRAM_CHAT_ID","TELEGRAM_BACKUP_ENABLED","AUTO_BACKUP_ENABLED","AUTO_BACKUP_TIMES"];
    const patch = {};
