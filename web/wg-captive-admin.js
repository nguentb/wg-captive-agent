...
const API_BASE='/api/v1';const API_ENDPOINTS={state:API_BASE+'/state',sync:API_BASE+'/sync',clientStatus:(ip)=>API_BASE+'/clients/'+encodeURIComponent(ip)+'/status',clientExpiry:(ip)=>API_BASE+'/clients/'+encodeURIComponent(ip)+'/expiry',settings:API_BASE+'/settings',relayAction:API_BASE+'/relay/action',relayImport:API_BASE+'/relay/import',backup:API_BASE+'/backup',restore:API_BASE+'/restore'};async function api(path,options={}){const res=await fetch(path,{headers:{'Content-Type':'application/json',...(options.headers||{})},...options});const data=await res.json();if(!res.ok||data.ok===false)throw new Error(data.error||'Request failed');return data}
